...
import { BiChevronRight as NextIcon, BiChevronLeft as PrevIcon, BiStar as StarIcon } from 'react-icons/bi';
...
const BlogFeaturedHero = ({ data }: BlogFeaturedProps) => {
  ...
  const defaultImage =
    'https://res.cloudinary.com/dfcwcfx5z/image/upload/v1717877267/djisamsoe/malbh7segwlqnpojldy1.jpg';
    ...
    <div className="relative overflow-hidden rounded-xl border shadow-lg dark:border-neutral-700">
      ...
      >
        <Image
          ...
          fill={true}
          sizes="100vw, 100vh"
          className="h-full w-full transform object-cover object-left-bottom transition-transform duration-300"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black opacity-90 transition-opacity duration-300"></div>
      </div>
        ...
        <div className="flex flex-col justify-between gap-6 p-6 sm:p-8">
          ...
          <div className="flex flex-col justify-end gap-6">
            <div className="flex flex-col space-y-2 text-white">
              <Link href={`/blog/${currentFeatured?.slug}?id=${currentFeatured?.id}`}>
              ...
              <p className="hidden sm:block">{formatExcerpt(currentFeatured?.description)}</p>
              <div className="flex gap-x-5 pt-1 text-neutral-400">
                <div className="flex items-center gap-1 ">
                  <DateIcon size={16} />
                  <span className="ml-0.5 text-xs">{formatDate(currentFeatured?.published_at)}</span>
                </div>
                <div className="flex items-center gap-1">
                  <ViewIcon size={15} />
                  <span className="ml-0.5 text-[13px]">
                    {currentFeatured?.page_views_count?.toLocaleString()} Views
                  </span>
                </div>
              </div>
            </div>
